...
/////////////////////////////////////////////////////////////////////////////////////////
// Assuming we want to use a static callback function, this is the constructor
// we would want to use
template<typename T>
FileWatcher<T> *FileWatcher<T>::StartFileWatch(
    std::string path,
...
{
    // call the static function constructor for a file watcher
    FileWatcher<T> *fw = new FileWatcher<T>(
        path, extension, mode, sleepTime, callback, recursive
        );

    // begin the thread for handling this directory
    fw->m_threadHandle = CreateThread(
        nullptr, 0, FileWatcher<T>::staticWatcherMain, fw, 0, &fw->m_threadID
        );

    // return file watcher to user
    return fw;
}
    ...
    )
{
    ...
    FileWatcher<T> *fw = new FileWatcher<T>(
        path, extension, mode, sleepTime, classFunc, callbackCaller, recursive
        );

    // call thread
    fw->m_threadHandle = CreateThread(
        nullptr, 0, FileWatcher<T>::classWatcherMain, fw, 0, &fw->m_threadID
        );
...
/////////////////////////////////////////////////////////////////////////////////////////
// constructor, takes the file extension, when to call func (only on new, when modified, 
// or both), a delay to check, fp.
template<typename T>
FileWatcher<T>::FileWatcher(
    std::string path,
    std::string extension,
    WatchMode mode,
    unsigned sleepTime,
...
/////////////////////////////////////////////////////////////////////////////////////////
// constructor, takes the file extension, when to call func (only on new, when modified, 
// or both), a delay to check, method fp, obj to call method
template<typename T>
FileWatcher<T>::FileWatcher(
    std::string path,
    std::string extension,
    WatchMode mode,
    unsigned sleepTime,
    void(T::*callbackFunc)(const std::string),
    T *callbackCaller,
    bool recursive
    )
    : m_mode( mode )
...
DWORD WINAPI FileWatcher<T>::staticWatcherMain( LPVOID lpArg )
{
    ...
    // we will search forever until we are manually told to stop
    while ( true )
    {
...
/////////////////////////////////////////////////////////////////////////////////////////
// uninitialize ourselves, stoopping our thread
template<typename T>
...
// recursive search
template<typename T>
bool FileWatcher<T>::scanDirectory(
    ...
    do
    {
            ...
            wcscmp( fileData.cFileName, L".." ) != 0 )
        {
            ...
            if ( (fileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) )
            {
                ...
                if ( m_recursive &&
                    scanDirectory( directory + L"\\" + fileData.cFileName, fileList ) )
                {
                    return true;
                }
            ...
            else
            {
                ...
                if ( wcsstr( fileData.cFileName, m_extension.c_str( ) ) != nullptr )
                {
                    ...
                    stat(
                        wToNStr( directory + L"\\" + fileData.cFileName ).c_str( ),
                        &fileInfo
                        );
                    ...
                    case ON_MODIFICATION:
                        ...
                        // if it's zero, it's a new file, add to map but don't call 
                            // callback on it
                        if ( m_fileList[ fileData.cFileName ] == 0 )
                        {
                    ...
                    // at this point, the file is added to the list of files that will 
                        // have the callback called on them
...
std::string FileWatcher<T>::wToNStr( const std::wstring &widestr )
{
    ...
    return convert.to_bytes( widestr );
}
